...
/**
 * jays_cake_Customizer_Notice_Section class
 */
class jays_cake_Customizer_Notice_Section extends WP_Customize_Section {
	/**
	...
	public function json() {
		$json = parent::json();
		global $jays_cake_recommended_plugins;
		...
		$show_recommended_plugins = get_option( 'jays_cake_show_recommended_plugins' );

		foreach ( $jays_cake_recommended_plugins as $slug => $plugin_opt ) {
			...
			$jays_cake_recommended_plugin['url']    = $this->create_action_link( $active['needs'], $slug );
			if ( $active['needs'] !== 'install' && $active['status'] ) {
				$jays_cake_recommended_plugin['class'] = 'active';
			} else {
				$jays_cake_recommended_plugin['class'] = '';
			}

			switch ( $active['needs'] ) {
				case 'install':
					$jays_cake_recommended_plugin['button_class'] = 'install-now button';
					$jays_cake_recommended_plugin['button_label'] = $install_button_label;
					break;
				case 'activate':
					$jays_cake_recommended_plugin['button_class'] = 'activate-now button button-primary';
					$jays_cake_recommended_plugin['button_label'] = $activate_button_label;
					break;
				case 'deactivate':
					$jays_cake_recommended_plugin['button_class'] = 'deactivate-now button';
					$jays_cake_recommended_plugin['button_label'] = $deactivate_button_label;
					break;
			}
			$info   = $this->call_plugin_api( $slug );
			$jays_cake_recommended_plugin['id'] = $slug;
			$jays_cake_recommended_plugin['plugin_slug'] = $slug;

			if ( ! empty( $plugin_opt['description'] ) ) {
				$jays_cake_recommended_plugin['description'] = $plugin_opt['description'];
			} else {
				$jays_cake_recommended_plugin['description'] = $info->short_description;
			}

			$jays_cake_recommended_plugin['title'] = $info->name;

			$customize_plugins[] = $jays_cake_recommended_plugin;
				...
				<div class="recomended-actions_container" id="plugin-filter">
					<# if( data.recommended_plugins.length > 0 ){ #>
						<# for (action in data.recommended_plugins) { #>
							<div class="jays-cake-recommeded-actions-container jays-cake-recommended-plugins" data-index="{{ data.recommended_plugins[action].index }}">
								<# if( !data.recommended_plugins[action].check ){ #>
									<div class="jays-cake-recommeded-actions">
